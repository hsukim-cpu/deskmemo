...
2. 拖最上緣移動位置、拉邊框調大小
3. 滑鼠移上去會浮出工具：
   - 左上**樣式**（無印筆記本風）：空白／橫線／**列表**（方框打勾劃掉）／**手寫**（筆／橡皮擦／清空）／**週記**（MONTH＋MON~SUN 橫帶）／**月記**（當月月曆格、可前後翻月、今天反白）／**單字卡**（鐵環活頁卡、正反面翻面背單字）
   - 樣式下方**五個色點**：換紙色（象牙白／純白／霧灰／灰綠／牛皮紙）
   - **文字工具**：字型（黑體／楷體／明體）、A−／A＋ 調字大小、**B 加粗、I 斜體、螢光筆**（先選取文字再按，只作用在選到的那段；Ctrl+B、Ctrl+I 也通）。手寫模式另有螢光筆畫筆
